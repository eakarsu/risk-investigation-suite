const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:3720';
...
const adminCookie = await login('admin@risk-investigation.local', 'admin123');
const managerCookie = await login('manager@risk-investigation.local', 'manager123');
const analystCookie = await login('analyst@risk-investigation.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/cases', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/cases', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/cases/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/cases/approve`, {
  method: 'POST',
  headers: {
